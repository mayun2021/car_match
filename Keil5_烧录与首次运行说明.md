...
6. 偏航角转过 350°（跑完一圈，纯角度判定，不再需要回到 A 标志）后显示
   `BRAKE`，再显示 `LAP DONE`，时间保持不变。
...
4. 停车位置和 A 点有偏差时改 `ROBOT_Q2_FINISH_YAW_DEG`（默认 350°）：
   停过 A 点就调小，没转够就调大，每次只改 5°；主动刹车时长
   `ROBOT_Q2_ACTIVE_BRAKE_MS`（默认 120 ms）影响的是刹车距离，不是触发时机。
