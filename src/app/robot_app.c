...
/**
 * @brief 判断循迹探头是否已经位于黑线附近，可以安全起步。
 */
static bool line_ready_for_start(void)
{
    return s_tel.line_valid &&
           s_tel.line_mask != 0x0Fu &&
           absf_local(s_tel.line_error) <= ROBOT_LINE_START_MAX_ERROR;
}
...
static void run_line_control(const line_sample_t *line, float dt_s, uint32_t now_ms)
{
    ...
    {
        /*
         * 丢线后按最近一次误差方向缓弯找线，两轮始终保持同向。
         * 禁止旧版“一个正转、一个反转”的原地疯狂旋转。
         */
        if (s_line_lost_since_ms == 0u)
        {
            s_line_lost_since_ms = now_ms;
        }
        else if (now_ms - s_line_lost_since_ms >= ROBOT_LINE_LOST_STOP_MS)
        {
            stop_task();
            return;
        }
...
void robot_app_tick(uint32_t now_ms)
{
    ...
    if (s_tel.state == ROBOT_STATE_RUNNING)
    {
        ...
        {
            if (s_tel.run_elapsed_ms <= ROBOT_START_SPIN_GUARD_MS &&
